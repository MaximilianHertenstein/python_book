...
└── book_theme/       # Custom mdBook theme
    ...
    ├── head.hbs          # Theme head template
    └── header.hbs        # Theme header with Thebe status element
```
...
A deployment workflow is configured in `.github/workflows/deploy.yml` to automatically build and deploy the book. It installs mdBook, mdbook-quiz, Node.js dependencies, copies Thebe assets, builds the book, and uploads the output for deployment.
