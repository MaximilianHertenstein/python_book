...
man sogenannte `if`-*Statements*. Diese gehören wie Funktionsdefinitionen
zu den zusammengesetzten *Statements*, da mit ihnen mehrere
*Statements* zu einem einzigen *Statement* verbunden werden.

## if-Statements

Bei diesen *Statements* schreibt man eine Bedingung bzw. einen
*booleschen Ausdruck* zwischen das Schlüsselwort `if` und einen
Doppelpunkt. Danach folgen eingerückte *Statements*. Diese werden nur
dann ausgeführt, wenn der Ausdruck zu `True` ausgewertet wird.
...
Im zweiten Fall wird `return` nicht erreicht. Deshalb wird auch nichts
zurückgegeben. Dies kann man ändern, indem man unter dem `if` ein
zweites `return` einbaut.
...
- Falls `points >= 50` zu `True` ausgewertet wird, wird `return 'Passed'`
  ausgeführt. Beim Ausführen von `return` verlässt die Funktion die
  Funktion. Deshalb wird `'Passed'` zurückgegeben und nichts weiter
  ausgeführt.
- Falls `points < 50` zu `True` ausgewertet wird, wird `return 'Passed'`
  übersprungen. Es geht direkt weiter mit `return 'Failed'`.
...
- Falls `points` größer als `75` ist, wird `return 'Very good'`
  ausgeführt. Beim Ausführen von `return` verlässt die Funktion die
  Funktion. Deshalb wird `'Very good'` zurückgegeben und nichts weiter
  ausgeführt.
